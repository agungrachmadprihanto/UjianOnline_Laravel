        ...
        <div id="menu-content" class="flex flex-col w-full pb-[30px]">
            ...
            <div class="flex flex-col px-5 mt-5">
                <div class="w-full flex justify-between items-center">
                    ...
                    <a href="{{ route('dashboardcourses.create') }}" class="h-[52px] p-[14px_20px] bg-[#6436F1] rounded-full font-bold text-white transition-all duration-300 hover:shadow-[0_4px_15px_0_#6436F14D]">Add New Course</a>
                </div>
            </div>
            <div class="course-list-container flex flex-col px-5 mt-[30px] gap-[30px]">
                ...
                <div class="list-items flex flex-nowrap justify-between pr-10">
                    <div class="flex shrink-0 w-[300px]">
                        <div class="flex items-center gap-4">
                            <div class="w-16 h-16 flex shrink-0 overflow-hidden rounded-full">
                                <img src="{{Storage::url($courses->cover)}}" class="object-cover" alt="thumbnail">
                            </div>
                            <div class="flex flex-col gap-[2px]">
                                <p class="font-bold text-lg">{{ $courses->name }}</p>
                                <p class="text-[#7F8190]">Beginners</p>
                            </div>
                        </div>
                    </div>
                    <div class="flex shrink-0 w-[150px] items-center justify-center">
                        <p class="font-semibold">{{ \Carbon\Carbon::parse($courses->created_at)->format('F j, Y') }}</p>
                    </div>
                    
                    @if($courses->category->name == 'Digital marketing')
                    ...
                    @elseif($courses->category->name == 'programming')
                    <div class="flex shrink-0 w-[170px] items-center justify-center">
                    ...
                    <div class="flex shrink-0 w-[120px] items-center">
                        <div class="relative h-[41px]">
                            <div class="menu-dropdown w-[120px] max-h-[41px] overflow-hidden absolute top-0 p-[10px_16px] bg-white flex flex-col gap-3 border border-[#EEEEEE] transition-all duration-300 hover:shadow-[0_10px_16px_0_#0A090B0D] rounded-[18px]">
                                ...
                                <a href="{{ route('dashboardcourses.show', $courses) }}" class="flex items-center justify-between font-bold text-sm w-full">
                                    Manage
                                </a>
                                <a href="course-students.html" class="flex items-center justify-between font-bold text-sm w-full">
                                    Students
                                </a>
                                <a href="{{ route('dashboardcourses.edit', $courses) }}" class="flex items-center justify-between font-bold text-sm w-full">
                                    Edit Course
                                </a>
                                <form action="{{ route('dashboardcourses.destroy', $courses->id) }}" method="POST">
                                    @method("DELETE")
                                    @csrf
